...
export async function configRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.put<{ Body: { actionType: string; thresholdAmount?: number | null; approverRoleId: string } }>(
    '/api/approval-workflows',
    {
      ...
      schema: { body: { type: 'object', required: ['actionType', 'approverRoleId'], additionalProperties: false, properties: {
    ...
    async (req, reply) =>
      withTenantContext(req.ctx, async (db) => {
        ...
        const { rows: role } = await db.query('SELECT id FROM roles WHERE id = $1', [req.body.approverRoleId]);
        ...
        const { rows } = await db.query(
          `INSERT INTO approval_workflows (tenant_id, action_type, threshold_amount, approver_role_id)
           ...
           RETURNING *`,
          [req.body.actionType, req.body.thresholdAmount ?? null, req.body.approverRoleId]);
        const r = rows[0];
